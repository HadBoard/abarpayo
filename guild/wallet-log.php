...
// ----------- get data ------------------------------------------------------------------------------------------------
$counter = 1;
$result=$action->guild_get_request();
// ----------- get data ------------------------------------------------------------------------------------------------

// ----------- delete --------------------------------------------------------------------------------------------------
if (isset($_GET['remove'])) {
    $id = $action->request('remove');
    $_SESSION['error'] = !$action->guild_request_remove($id);
...
<div class="page-wrapper">
    ...
    <div class="row page-titles">
        ...
        <div class="col-md-12 align-self-center text-right">
            <h3 class="text-primary">کیف پول</h3></div>
        <div class="col-md-12 align-self-center text-right">
            <ol class="breadcrumb">
    ...
    <div class="container-fluid">
        ...
        <!-- ----------- add button ---------------------------------------------------------------------------- -->
        <div class="row">
            <a class="add-user mb-2" href="<?= $main_url ?>"> ثبت درخواست تسویه <i class="fas fa-plus"></i></a>
        </div>
        <!-- ----------- add button ---------------------------------------------------------------------------- -->
                                    ...
                                    <table id="example23" class="display nowrap table table-hover table-striped"
                                        ...
                                        if(mysqli_num_rows($result)){
                                            ...
                                            <tr class="text-center">
                                                ...
                                                <td class="text-center"><?= $action->cart_get($row->cart_id)->title." | ".$action->cart_get($row->cart_id)->cart_number ?></td>
                                                ...
                                                <td class="text-center">
                                                   <? if(!$row->status){?>
                                                    ...
                                                    <?}else{?>
                                                        <?=$action->time_to_shamsi($row->paymented_at)?>
                                                    <?}?>
                                                </td>
